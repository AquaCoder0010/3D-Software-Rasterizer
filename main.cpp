...
int main() {
  while (true) {
    ...
    // objects for view
    sf::VertexArray mesh_vertex(sf::Lines, mesh.tri_count * 6);
    mesh_points = new sf::CircleShape[mesh.tri_count * 3];
    float radius = 3.f;
    ...
    pair_list = new IndexPair[mesh.tri_count * 3];
    ...
    while (window.isOpen() == true) {
      while (window.pollEvent(event)) {
        ...
        if (event.type == sf::Event::MouseButtonPressed &&
          ...
          if (clickedIndex != -1) {
            ...
            Vec3 targetPos =
                mesh.tri_list[targetTriIndex].points[targetVertIndex];

            int pair_list_index = 0;
            for (int t = 0; t < mesh.tri_count; t++) {
              for (int v = 0; v < 3; v++) {
                Vec3 p = mesh.tri_list[t].points[v];
      ...
      if (isDragging && pair_list_total_size != 0) {
        ...
        for (int i = 0; i < pair_list_total_size; i++) {
          ...
          mesh.tri_list[t].points[v].x += modelDelta.x;
          mesh.tri_list[t].points[v].y += modelDelta.y;
          mesh.tri_list[t].points[v].z += modelDelta.z;
        }
      }
      ...
      for (int i = 0; i < mesh.tri_count; i++) {
        Triangle curr_tri = mesh.tri_list[i];
        Triangle proj_tri;
